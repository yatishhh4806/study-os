...
import { Brain, Trophy, Clock, Flame, TrendingUp, Target, X, CalendarRange } from "lucide-react";
...
const SAMPLE_SESSIONS = [
  { id: "1", type: "focus", date: "2026-06-30", startTime: "08:10", durationMin: 25, plannedMin: 25, distractions: 0, completed: true },
  { id: "2", type: "focus", date: "2026-06-30", startTime: "19:05", durationMin: 25, plannedMin: 25, distractions: 1, completed: true },
  { id: "3", type: "focus", date: "2026-06-30", startTime: "19:40", durationMin: 52, plannedMin: 50, distractions: 0, completed: true },
  { id: "4", type: "focus", date: "2026-06-30", startTime: "20:45", durationMin: 18, plannedMin: 25, distractions: 2, completed: false },
  { id: "5", type: "focus", date: "2026-06-29", startTime: "21:00", durationMin: 25, plannedMin: 25, distractions: 1, completed: true },
  { id: "6", type: "focus", date: "2026-06-28", startTime: "18:00", durationMin: 75, plannedMin: 75, distractions: 0, completed: true },
  { id: "7", type: "focus", date: "2026-06-27", startTime: "09:00", durationMin: 50, plannedMin: 50, distractions: 1, completed: true },
  { id: "8", type: "focus", date: "2026-06-26", startTime: "20:00", durationMin: 30, plannedMin: 50, distractions: 3, completed: false },
  { id: "9", type: "focus", date: "2026-06-25", startTime: "17:30", durationMin: 100, plannedMin: 100, distractions: 0, completed: true },
  { id: "10", type: "focus", date: "2026-06-24", startTime: "10:00", durationMin: 25, plannedMin: 25, distractions: 0, completed: true },
];
...
function generateInsights(stats, sessions) {
  ...
  if (stats.longestSession > 0) {
    insights.push(`Your longest focus streak was ${stats.longestSession} minutes.`);
  }
  ...
  if (abandoned > 0) {
    insights.push(`You abandoned ${abandoned} session${abandoned === 1 ? "" : "s"} today before it finished.`);
  }

  if (stats.distractions > 0) {
    const rate = todays.length > 0 ? (stats.distractions / todays.length).toFixed(1) : 0;
    insights.push(`You averaged ${rate} distraction${rate === "1.0" ? "" : "s"} per session today.`);
  }

  if (stats.consistency >= 90) {
    insights.push(`You're highly consistent — ${stats.consistency}% of planned focus time completed.`);
  } else if (stats.consistency > 0) {
    insights.push(`You're at ${stats.consistency}% consistency today — small room to improve.`);
  }
...
export default function FocusAnalytics({ sessions = SAMPLE_SESSIONS }) {
  ...
  const goalProgress = targetMinutes > 0 ? Math.min(Math.round((stats.focusMinutes / targetMinutes) * 100), 100) : 0;
  const remainingMin = Math.max(targetMinutes - stats.focusMinutes, 0);
  const remainingLabel =
    remainingMin === 0 ? "Goal reached" : `${Math.floor(remainingMin / 60)}h ${remainingMin % 60}m left`;
      ...
      >
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 16, fontWeight: 800, color: "#fff" }}>{goalProgress}%</span>
          </div>
        ...
        >
          <span style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 14.5, color: "rgba(255,255,255,0.8)" }}>
            <Trophy size={17} color={accent} />
            Sessions Completed
          </span>
          <span style={{ fontSize: 22, fontWeight: 800, color: "#fff" }}>{stats.sessionsCompleted}</span>
        </div>
        ...
        >
          ...
          <span style={{ fontSize: 22, fontWeight: 800, color: "#fff" }}>{stats.focusTimeLabel}</span>
        </div>
        ...
        >
          ...
          <span style={{ fontSize: 22, fontWeight: 800, color: "#fff" }}>{stats.longestSession}m</span>
        </div>
        ...
        >
          <span style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 13, color: "rgba(255,255,255,0.5)", marginBottom: 4 }}>
            <TrendingUp size={15} color={accent} />
            Peak Focus Period
          </span>
          <div style={{ fontSize: 19, fontWeight: 800, color: "#fff" }}>{stats.peakPeriod}</div>
        </div>
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 84 }}>
            {weeklyReport.map((d) => {
              const h = Math.max((d.minutes / maxDayMin) * 100, d.minutes > 0 ? 6 : 2);
              ...
              return (
                <div key={d.date} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
                  ...
                  />
                  <span style={{ fontSize: 10.5, color: "rgba(255,255,255,0.4)", marginTop: 6 }}>{d.label[0]}</span>
                </div>
              );
            })}
          </div>
        ...
        >
          <div
            ...
            style={{
              width: "100%",
              maxWidth: 320,
              background: "linear-gradient(180deg, rgba(24,16,32,0.97), rgba(10,7,15,0.98))",
              border: `1px solid ${accent}55`,
              borderRadius: 20,
              padding: 24,
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
              <span style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 17, fontWeight: 800, color: "#fff" }}>
            ...
            <label style={{ fontSize: 13, color: "rgba(255,255,255,0.55)", marginBottom: 8, display: "block" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
              ...
              />
              <span style={{ fontSize: 14, color: "rgba(255,255,255,0.5)" }}>hours / day</span>
            </div>
              ...
              {[2, 4, 6, 8].map((h) => (
                <button
                  ...
                  style={{
                    ...
                    padding: "8px 0",
                    borderRadius: 10,
                    border: draftTarget === String(h) ? `1px solid ${accent}` : "1px solid rgba(255,255,255,0.1)",
                    background: draftTarget === String(h) ? "rgba(168,85,247,0.15)" : "transparent",
                    color: draftTarget === String(h) ? "#fff" : "rgba(255,255,255,0.5)",
                    fontSize: 13,
                    fontWeight: 700,
                    cursor: "pointer",
                  }}
