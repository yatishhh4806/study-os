...
export async function spotifyRequest(user, path, options = {}) {
  ...
  console.log("SPOTIFY RESPONSE STATUS:", response.status, path);
...
export async function getSpotifyProfile(accessToken) {
  ...
  return parseSpotifyResponse(response, "Failed to fetch Spotify profile");
}
...
export async function getUserPlaylists(user) {
  const data = await spotifyRequest(user, "/me/playlists?limit=50");

  return data.items || [];
}

export async function getPlaylist(user, playlistId) {
  return spotifyRequest(user, `/playlists/${playlistId}`);
}

export async function searchPlaylists(user, query, limit = 10) {
  ...
  return spotifyRequest(user, `/search?${params.toString()}`);
}
...
export async function pause(user) {
  return spotifyRequest(user, "/me/player/pause", {
    method: "PUT",
  });
}

export async function nextTrack(user) {
  return spotifyRequest(user, "/me/player/next", {
    method: "POST",
  });
}

export async function previousTrack(user) {
  return spotifyRequest(user, "/me/player/previous", {
    method: "POST",
  });
}

export async function seek(user, position) {
  return spotifyRequest(user, `/me/player/seek?position_ms=${position}`, {
    method: "PUT",
  });
}

export async function setVolume(user, volume) {
  return spotifyRequest(user, `/me/player/volume?volume_percent=${volume}`, {
    method: "PUT",
  });
}
